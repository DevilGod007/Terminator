...
def save_game_pgn():
    """Save game with proper result for RL training."""
    game = chess.pgn.Game()
    ...
    # Set the actual game result for RL training
    if "Draw" in winner:
        game.headers["Result"] = "1/2-1/2"
    elif "White" in winner:
        game.headers["Result"] = "1-0"
    elif "Black" in winner:
        game.headers["Result"] = "0-1"
    else:
        game.headers["Result"] = "*"  # Game in progress
    
    # Add all moves to the game
    node = game
    ...
    print(f"Game saved to {game_file} with result: {game.headers['Result']}")
